...
fn check_and_warn_config(config: &GenericConfiguration) -> Result<(), GenericError> {
        ...
        .error_context("Unable to flatten configuration into a list of dot-separated keys.")?
    {
        ...
        // TODO: provide the correct warning text
        // ```text
        // +--------------------------------+--------------------------------------+------------------------+
        // | Key                            | Description                          | Toby Note              |
        // +--------------------------------+--------------------------------------+------------------------+
        // | telemetry.enabled              | Enables internal agent telemetry as  | A lot of internal      |
        // |                                | custom metrics (default false).      | telemetry centers      |
        // |                                | Saluki uses                          | around DSD, so it      |
        // |                                | data_plane.telemetry_enabled         | strikes me that maybe  |
        // |                                | instead. Observability-of-the-agent  | we _do_ want this, but |
        // |                                | feature, not a data pipeline         | it also would add a    |
        // |                                | feature.                             | _buttload_ of          |
        // |                                |                                      | telemetry, and there's |
        // |                                |                                      | also some subtlety     |
        // |                                |                                      | around our telemetry   |
        // |                                |                                      | and the filter level.. |
        // |                                |                                      | so it strikes me that  |
        // |                                |                                      | we could warn but with |
        // |                                |                                      | a custom message on    |
        // |                                |                                      | how to enable ADP      |
        // |                                |                                      | telemetry / filter     |
        // |                                |                                      | levels.                |
        ...
        // telemetry.enabled=true is a special case, return a warning with a custom message on how to enable ADP telemetry and filter levels.
